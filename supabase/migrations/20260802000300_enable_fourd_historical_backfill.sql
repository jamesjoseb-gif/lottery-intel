-- Extend the verified 4D rule set to the first result exposed by the official archive.
update public.game_rule_sets
set effective_from = date '1986-05-31'
where game_code = '4d' and version = 1 and effective_from > date '1986-05-31';
...
begin
  ...
  if p_draw_date < date '1986-05-31' or p_draw_date > current_date then
    raise exception 'draw date is outside the supported official archive window';
  end if;
  ...
  or (select count(*) from jsonb_to_recordset(p_results) as r(prize_type text, position smallint) where prize_type in ('first','second','third') and position <> 1) <> 0
  or (select count(distinct position) from jsonb_to_recordset(p_results) as r(prize_type text, position smallint) where prize_type = 'starter' and position between 1 and 10) <> 10
  or (select count(distinct position) from jsonb_to_recordset(p_results) as r(prize_type text, position smallint) where prize_type = 'consolation' and position between 1 and 10) <> 10
  ...
  if exists (select 1 from public.draws where game_code = '4d' and draw_no = btrim(p_draw_no) and draw_date <> p_draw_date) then
    raise exception 'draw % already exists with a different draw date', p_draw_no;
  end if;
  ...
  values
    (v_draw_id, 1, v_rule_set_id, 'eligible', p_checksum,
     'singapore-pools-4d-v1', '2', p_import_run_id, 'Singapore Pools V1 import')
