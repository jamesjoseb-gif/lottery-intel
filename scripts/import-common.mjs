...
export async function runImporter(config) {
  ...
  const matched = candidates.filter((row) => (!requestedDraw || row.drawNo === requestedDraw) && (!from || row.drawDate >= from) && (!to || row.drawDate <= to));
  const selected = requestedDraw || from || to ? matched : matched.slice(0, 1);
  if (!selected.length) throw new Error("No official draws matched the requested draw/range.");
  const started = new Date().toISOString();
  const runRes = await api("import_runs", key, { method: "POST", body: JSON.stringify({ mode: requestedDraw ? "single" : "backfill", game_code: config.game, requested_from: from, requested_to: to, status: "running", created_by: config.importer, started_at: started, config: { source: config.listUrl, draw_no: requestedDraw ?? null } }) });
